...
// MIDI.js `MidiAudio` API that Player.svelte uses (empty / loadPlugin / noteOn /
// noteOff / programChange / stopAllNotes). Swapping the backend here keeps the
// .svelte diff minimal. Ported from klstudio's app/inc/fluidAudio.ts.
//
// Architecture: Player.svelte drives playback with a look-ahead loop — it sends
// noteOn/noteOff with an absolute performance.now()-based timestamp each note
// should sound. We forward those to FluidSynth's Sequencer via
// sendEventAt(event, dtMs, isAbsolute=false), i.e. "play in dtMs milliseconds",
// which preserves the look-ahead timing without reconciling performance.now()
// against the sequencer's own tick origin.
//
// Fallback: the FluidSynth soundfont is large (~40 MB) and takes a moment to
// fetch + decode. While it loads, we fall back to the legacy MIDI.js WebAudio
// player (a small, locally-bundled acoustic-grand-piano soundfont) so playback
// is audible immediately. Once FluidSynth is ready we switch to it
// transparently.
//
// SvelteKit notes vs. the klstudio (Vue) original:
//  - The legacy MidiAudio comes from the music-widgets UMD bundle on `window`,
//    not an npm import, so it is injected via init() rather than imported here.
//  - Static asset URLs are resolved against the SvelteKit `base` path, also
//    passed in through init().

// Minimal shape of the legacy music-widgets MidiAudio we rely on.
interface LegacyMidiAudioLike {
	WebAudio: {
		empty(): boolean;
		needsWarmup?(): boolean;
		awaitWarmup?(): Promise<void>;
	};
	loadPlugin(opts: { soundfontUrl: string; api: string }): Promise<void>;
...
// SoundFont filename (served from static/soundfont/). Swap this file to upgrade
// audio quality — no code change needed. A General MIDI .sf2/.sf3 gives 128
// instruments; the legacy single-piano soundfont made every program sound as
// piano. Currently MuseScore_General GM (~40 MB, .sf3; ogg-compressed samples —
// requires the libfluidsynth with-libsndfile build below).
const SOUNDFONT_FILE = 'soundfont/gm.sf3';

// FluidSynth WASM runtime, copied into static/fluid/. The with-libsndfile build
// is required to decode .sf3 (ogg-compressed) soundfonts; it also reads .sf2.
const LIBFLUIDSYNTH_FILE = 'fluid/libfluidsynth-2.4.6-with-libsndfile.js';
const WORKLET_FILE = 'fluid/js-synthesizer.worklet.js';
...
let legacy: LegacyMidiAudioLike | null = null;
let legacyReady = false;

// Base path for static assets (SvelteKit `base`), set in init().
let baseUrl = '';

const assetUrl = (file: string): string => `${baseUrl}/${file}`;
...
// Inject the legacy UMD MidiAudio and the SvelteKit base path. Must be called
// once before loadPlugin().
const init = (legacyMidiAudio: LegacyMidiAudioLike, base: string): void => {
	legacy = legacyMidiAudio;
	baseUrl = base;
};
...
const loadPlugin = async (): Promise<void> => {
	...
	if (legacy && legacy.WebAudio.empty()) {
		legacy
			.loadPlugin({ soundfontUrl: `${baseUrl}/soundfont/`, api: 'webaudio' })
	...
	loadingPromise = (async () => {
		...
		await audioCtx.audioWorklet.addModule(assetUrl(LIBFLUIDSYNTH_FILE));
		await audioCtx.audioWorklet.addModule(assetUrl(WORKLET_FILE));
		...
		const sfontBuffer = await (await fetch(assetUrl(SOUNDFONT_FILE))).arrayBuffer();
...
const resume = async (): Promise<void> => {
	...
	if (legacyReady && legacy && legacy.WebAudio.needsWarmup?.())
		await legacy.WebAudio.awaitWarmup?.();
};
...
const noteOn = (channel: number, note: number, velocity: number, timestamp: number): void => {
	...
	if (legacyReady && legacy)
		legacy.noteOn(channel, note, velocity, timestamp);
};
...
const noteOff = (channel: number, note: number, timestamp: number): void => {
	...
	if (legacyReady && legacy)
		legacy.noteOff(channel, note, timestamp);
};
